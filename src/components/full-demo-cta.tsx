...
const FEATURES = [
  { icon: LayoutDashboard, key: 'fullDemo.feature1' },
  { icon: Users, key: 'fullDemo.feature2' },
  { icon: Activity, key: 'fullDemo.feature3' },
  { icon: FileText, key: 'fullDemo.feature4' },
] as const;

interface FullDemoCtaProps {
  className?: string;
...
export function FullDemoCta({
  className,
  variant = 'section',
  trackingId = 'full_demo_open',
}: FullDemoCtaProps) {
  const { t } = useLanguage();
  ...
  const handleOpenDemo = () => {
    trackCta(trackingId);
    openAppDemo(trackingId);
  };

  const card = (
    <Card
      className={cn(
        'relative overflow-hidden border-blue-400/25 bg-gradient-to-br from-blue-500/8 via-card to-cyan-500/5 shadow-lg shadow-blue-500/5',
        'dark:border-blue-500/30 dark:from-blue-950/30 dark:via-card dark:to-cyan-950/10',
        className
      )}
    >
      ...
      <CardContent className="relative p-6 sm:p-10 lg:p-12">
        <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <div>
            <Badge
              variant="outline"
              className="mb-4 border-blue-500/30 bg-blue-500/10 text-blue-600 dark:text-blue-400"
            >
            ...
            <p className="mt-3 text-sm text-muted-foreground/90">{t('fullDemo.note')}</p>
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center">
              ...
              <span className="text-xs text-muted-foreground sm:text-sm">{t('fullDemo.ctaHint')}</span>
            </div>
          </div>
